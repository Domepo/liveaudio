...
describe("AdminLoginCard", () => {
  it("keeps submit button disabled until name and password are set", async () => {
    ...
    component.$on("adminNameChange", async (event) => {
      await component.$set({ adminName: event.detail.value });
    });
    component.$on("adminPasswordChange", async (event) => {
      await component.$set({ adminPassword: event.detail.value });
    });
